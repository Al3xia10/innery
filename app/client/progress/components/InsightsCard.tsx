  ...
  return (
    <SoftCard
      ...
      right={
        <button
          ...
          className="inline-flex items-center justify-center rounded-xl border border-black/5 bg-(--color-card) px-3 py-2 text-sm font-semibold text-foreground shadow-[0_4px_10px_rgba(31,23,32,0.05)] hover:bg-(--color-soft) transition"
        >
    ...
    >
      {!open ? (
        ...
        <div className="rounded-2xl border border-dashed border-black/10 bg-(--color-card) p-5">
          ...
          <Link
            href="/client"
            className="mt-4 inline-flex w-full sm:w-auto items-center justify-center rounded-xl bg-(--color-primary) px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:opacity-95 transition"
          >
        ...
        <>
          <p className="text-sm text-(--color-foreground-muted,#6B5A63)">
            Dacă ceva de aici nu se potrivește, e în regulă. Tu îți cunoști contextul cel mai bine.
          </p>

          <ul className="mt-4 space-y-2 text-sm text-foreground">
            {insights.slice(0, 2).map((ins) => (
              <li key={ins.id} className="rounded-2xl border border-black/5 bg-(--color-card) p-4 shadow-[0_4px_10px_rgba(31,23,32,0.05)]">
                • {ins.text}
              </li>
            ))}
          </ul>

          <p className="mt-4 text-xs leading-relaxed text-(--color-foreground-muted,#6B5A63)">
            Nu e un verdict. Doar orientativ. Contextul rămâne mai important decât scorul.
          </p>
        </>
      ) : (
        <ul className="space-y-2 text-sm text-foreground">
          <li className="rounded-2xl border border-black/5 bg-(--color-card) p-4 shadow-[0_4px_10px_rgba(31,23,32,0.05)]">
